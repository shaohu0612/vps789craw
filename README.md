...
输出格式统一为：`CF优选IP:端口#备注-后缀`

| 采集源 | 条件分支 | 格式化输出示例 | 说明 |
| :--- | :--- | :--- | :--- |
| **优选域名** (`remarks=domain`) | 域名且端口为 `443` | `www.shopify.com#www.shopify.com-vps789` | 443 端口省略；备注为空则复用域名 |
| **优选域名** (`remarks=domain`) | 域名且端口为 `443`，有备注 | `dnew.cc#yong-vps789` | 443 端口省略；拼接备注与 `-vps789` |
| **优选域名** (`remarks=domain`) | 域名且端口不为 `443` | `cdn.test.com:8443#cdn.test.com-vps789` | 保留 `:端口` |
| **优选域名** (`remarks=domain`) | 若节点为 IP 地址 | `104.21.20.210:443#104.21.20.210-vps789` | 若为 IP 地址，`:端口` 绝不可省略 |
| **优选 IP** (`remarks=ip`) | IP 地址且端口为 `443` | `172.67.212.221:443#172.67.212.221-vps789` | `:端口` 绝不可省略；备注为空则复用 IP |
